...
def get_price(pair):    # Order Parameter
    tries = 3
        ...
        try:
            ...
            return price
                ...
                if i < tries - 1:  # i is zero indexed
                    print("NetworkError , try get_price :" + str(i) + " chances" + str(e))
                    time.sleep(0.5)
                    continue
                else:
                    print(str(e))
...
def create_buy_order(pair, buy_size, buy_price):
    # Order Parameter
    tries = 3
    for i in range(tries):
        try:
            post_only=True
            return exchange.create_limit_buy_order(pair,buy_size,buy_price,{'postOnly': post_only})
        except ccxt.NetworkError as e:
                if i < tries - 1:  # i is zero indexed
                    print("NetworkError , try create_buy_order :" + str(i) + " chances" + str(e))
                    time.sleep(0.5)
                    continue
                else:
                    print(str(e))
                ...
                if i < tries - 1:  # i is zero indexed
                    ...
                    raise
def create_sell_order(pair, sell_size, sell_price):
    # Order Parameter
    tries = 3
    for i in range(tries):
        try:
            post_only=True
            return exchange.create_limit_sell_order(pair,sell_size,sell_price,{'postOnly': post_only})
        except ccxt.NetworkError as e:
                if i < tries - 1:  # i is zero indexed
                    print("NetworkError , try create_sell_order: " + str(i) + " chances" + str(e))
                    time.sleep(0.5)
                    continue
                else:
                    print(str(e))
...
def get_order(id):    # Order Parameter
    tries = 3
    for i in range(tries):
        try:
            id = str(int(id))
            return exchange.fetchOrder(id)
        except ccxt.NetworkError as e:
                if i < tries - 1:  # i is zero indexed
                    print("NetworkError , try get_order :" + str(i) + " chances" + str(e))
                    time.sleep(0.5)
                    continue
                else:
                    print(str(e))
...
def grid_zone_calculation(up_zone, low_zone, grid_qty, capital, leverage):
    ...
    grid_size = float(lev_capital/grid_qty) #จำนวนการซื้อ
    step = float((up_zone - low_zone) / grid_qty) #ระยะห่าง
    total_zone = grid_qty #จำนวนเส้น zone
    ...
    for i in range(total_zone + 1):
        ...
        if price < up_zone: #ถ้า ราคา ต่ำกว่า ราคาบนสุด
            cumu_value += grid_size # บวกด้วยจำนวนการซื้อ
            grid_price.append(price) # เก็บราคา 
            grid_asset.append(cumu_value) #เก็บว่าซื้อโซนนี้ เท่าไหร่  จะประยุกต์ใช้กับ Normal Dirturtion
        elif price == up_zone: #สำหรับรอบแรก
            cumu_value += 0 
            grid_price.append(price)
            grid_asset.append(cumu_value)
...
def my_grid_zone_calculation(up_zone, low_zone,grid_qty):
    grid_zone = np.linspace(up_zone,low_zone,grid_qty)
    return grid_zone
